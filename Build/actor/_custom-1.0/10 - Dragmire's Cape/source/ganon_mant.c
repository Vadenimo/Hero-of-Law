...
ActorInitExplPad En_Ganon_Mant_Profile = {
    /**/ 0xDEAD,
    /**/ ACTORCAT_BOSS,
    /**/ 0x00000030,
...
void EnGanonMant_DrawCloak(PlayState* play, EnGanonMant* this) 
{
    ...
    MATRIX_FINALIZE_AND_LOAD(play->state.gfxCtx->polyOpa.p++, play->state.gfxCtx, "../z_en_ganon_mant.c", 572);

    // set texture
    gSPDisplayList(play->state.gfxCtx->polyOpa.p++, gMantMaterialDL);

    // set vertices, vertices are double buffered to prevent
    // modification of vertices as they are being drawn
    if (this->frameTimer % 2 != 0) {
        gSPSegment(play->state.gfxCtx->polyOpa.p++, 0x0C, gMant1Vtx);
    } else {
        gSPSegment(play->state.gfxCtx->polyOpa.p++, 0x0C, gMant2Vtx);
    }

    // draw cloak
    gSPDisplayList(play->state.gfxCtx->polyOpa.p++, gMantDL);
}
