...
s16 Fixed_atan2s(float y, float x);
void Euler2Quat(const Vec3s *r, Quaternion *q);
void Quat2Euler(const Quaternion *q, Vec3s *r);
...
void Quat2Euler(const Quaternion *q, Vec3s *r)
{
	...
	// (Skawo:) And then I found out this can still happen despite the above check, for some reason!
	// Hooray. This second check here fixes it for sure, though.
	// Hopefully.
	if (mult == 0)
		mult = 2000;
	else
